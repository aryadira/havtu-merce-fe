...
export const getOrders = async (page = 1, limit = 10): Promise<OrdersListResponse> => {
    const response = await api.get('/orders', { params: { page, limit } });
    return response.data;
};

export const useGetOrders = (page: number, limit: number, params: UseGetOrdersParams = {}) => {
    return useQuery({
...
export const getOrdersManage = async (page = 1, limit = 10): Promise<OrdersListResponse> => {
    const response = await api.get('/orders', { params: { page, limit } });
    return response.data;
};
