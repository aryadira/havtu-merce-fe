...
function CheckoutContent() {
    ...
    const { data: product, isLoading } = useProductShopDetail(pid as string);

    // Get selected item from product data
    const selectedItem = product?.items?.find((i: any) => i.id === itemId) || product?.items?.[0];

    // Dummy user address based on screenshot
    const [shippingForm, setShippingForm] = useState({
        street: 'Jl. Tipar Cakung KP. Baru RT.002 RW.007 no.223 Cakung Barat',
        city: 'Cakung',
        state: 'DKI Jakarta',
        postal_code: '13910',
        country: 'Indonesia',
        phone: '62895359783013',
    });

    const [shippingMethodId, setShippingMethodId] = useState(
        'e1fdd027-5ace-4607-8a5c-cb6bac0634e9',
    ); // Default JNE
    const [userPaymentMethodId, setUserPaymentMethodId] = useState(
        '2ea3e363-0f37-4a59-89b8-c26fe45db739',
    ); // Default BCA
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col gap-4 sticky top-24">
                        ...
                        <Card className="p-5 border border-gray-200">
                            ...
                            <div className="flex flex-col gap-4">
                                {[
                                    { id: '1', name: 'Mandiri Virtual Account', default: false },
                                    {
                                        id: '2ea3e363-0f37-4a59-89b8-c26fe45db739',
                                        name: 'BCA Virtual Account',
                                        default: true,
                                    },
                                    { id: '3', name: 'BRI Virtual Account', default: false },
                                    { id: '4', name: 'BNI Virtual Account', default: false },
                                ].map((pm) => (
                                    <div
                                        key={pm.id}
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-6 bg-gray-100 rounded flex items-center justify-center text-[10px] font-bold text-gray-400 border border-gray-200">
                                                {pm.name.split(' ')[0]} {/* Fake logo text */}
                                            </div>
                                            <span className="text-[15px]">{pm.name}</span>
                                        </div>
                                        <div
